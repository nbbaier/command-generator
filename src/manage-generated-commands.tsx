...
export default function Command() {
  ...
  return (
    <List isLoading={isLoading} searchBarPlaceholder="Search generated commands...">
...
function CommandForm({ spec, onSubmit }: { spec: CommandSpec; onSubmit: (values: Record<string, unknown>) => void }) {
  ...
  return (
    <Form
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Run Command" onSubmit={() => onSubmit(values)} />
        </ActionPanel>
      }
    >
          ...
          case "textfield":
            return (
              <Form.TextField
                ...
                onChange={(value) => setValues({ ...values, [input.id]: value })}
              />
            );
          case "textarea":
            return (
...
function CommandExecutor({ spec, formValues }: { spec: CommandSpec; formValues: Record<string, unknown> }) {
  const [isLoading, setIsLoading] = useState(true);
...
function ResultsList({ spec, data }: { spec: CommandSpec; data: Record<string, unknown> | null }) {
  if (!data || !spec.ui.dataSource) {
    return <List><List.EmptyView title="No data" /></List>;
  }

  const items = data[spec.ui.dataSource];
  if (!Array.isArray(items)) {
    return <List><List.EmptyView title="Invalid data format" description="Data source is not an array" /></List>;
  }
              ...
              spec.actions ? (
                <ActionPanel>
                  {spec.actions.map((action, actionIndex) => (
                    <ActionForItem key={actionIndex} action={action} item={item} context={itemContext} />
                  ))}
                </ActionPanel>
              ) : undefined
            }
...
function ResultsDetail({ spec, data }: { spec: CommandSpec; data: Record<string, unknown> | null }) {
...
function ActionForItem({
  ...
  action: { title: string; icon?: string; operation: { type: string; config: Record<string, unknown> } };
  item: unknown;
  ...
  return <Action title={action.title} icon={action.icon || Icon.Dot} onAction={handleAction} />;
}
